<div id="{{ $id ?? 'dropzone-wrapper' }}" class="flex items-center justify-center w-full">
    <label for="{{ $inputId ?? 'dropzone-file' }}"
            ...
            <div class="flex flex-col items-center justify-center pt-5 pb-6 px-4 text-center">
                ...
                <div id="{{ $uploadPromptId ?? 'upload-prompt' }}">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        @if(isset($accept) && $accept === 'image/*')
                            Images (PNG, JPG, GIF, SVG, WEBP)
                        @elseif(isset($accept))
                            {{ str_replace(',', ', ', str_replace('.', '', $accept)) }}
                        @else
                            SVG, PNG, JPG or GIF
                        @endif
        ...
        @endif
        <input 
            id="{{ $inputId ?? 'dropzone-file' }}" 
            type="file" 
            name="{{ $name ?? 'file' }}"
            class="hidden" 
            accept="{{ $accept ?? '.svg,.png,.jpg,.jpeg,.gif,.webp' }}"
        ...
        function handleFiles(files) {
            ...
            @isset($maxSize)
                ...
                if (file.size > maxSizeBytes) {
                    alert("File size exceeds maximum allowed size of {{ $maxSize }}");
                    return;
                }
            @endisset
