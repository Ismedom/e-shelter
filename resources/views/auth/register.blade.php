...
<x-layouts.layout>
    <div class="flex min-h-screen flex-col justify-center items-center px-6 py-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-sm">
            ...
            <h2 class="mt-6 text-center text-2xl/9 font-bold tracking-tight text-gray-700 dark:text-gray-300">{{trans('signup')}}</h2>
        </div>       
        <div class="mt-4 w-[calc(100%-20px)] sm:mx-auto sm:w-full sm:max-w-sm">
            <form class="space-y-5" method="POST" action="{{ route('register') }}">
                ...
                <div>
                    <div class="flex items-center justify-between mt-1">
                        <label for="confirm_password" class="block text-sm/6 font-medium text-gray-500 dark:text-gray-300">{{trans('confirm_password')}}</label>
                    </div>
                    <div>
                        <input 
                            type="password" 
                ...
                {{--  --}}
                <div>
                    <x-buttons.button type="submit" variant="primary" class="block w-full text-center">{{trans('signup')}}</x-buttons.button>
                </div>
            </form>
